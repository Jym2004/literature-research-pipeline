#!/usr/bin/env python3
"""Push Google Scholar paper data to Zotero via the local Connector API.
...
def build_zotero_item(paper):
    """Build a Zotero item from normalized Google Scholar paper data."""
    ...
    item = {
        ...
        'libraryCatalog': paper.get('libraryCatalog', 'Google Scholar'),
        'accessDate': now,
...
def save_items(items, uri=''):
    ...
    for i, item in enumerate(items):
        if 'id' not in item:
            item['id'] = f'scholar_{session_id}_{i}'
...
def download_pdf(pdf_url, timeout=PDF_DOWNLOAD_TIMEOUT):
    ...
    req = urllib.request.Request(pdf_url, headers={
        'User-Agent': 'literature-research-pipeline/1.0',
...
def main():
    ...
    for i, (paper, item) in enumerate(zip(papers, items)):
        ...
        item_id = item.get('id', f'scholar_{session_id}_{i}')
